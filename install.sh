...

{
  cat <<'DOTFILES_HDR'
# Auto-generated by install.sh — do not edit.
# Run `dotfiles` to list all custom commands from my-dot-files.
...
List all custom commands provided by my-dot-files, grouped by category.
Commands marked with * support --help / -h.
HELP
    return 0
  fi
  cat <<'__DOTFILES_LIST__'
Custom commands from my-dot-files  (* = supports --help)
